...
class SpectrumSN(object):
    ...
    def __init__(self, spec1D, z=0, SN_name="ZTF"):
        ...
        self.spec_name = spec1D[spec1D.find(SN_name) : spec1D.find(".ascii")]
...
class SpectrumSN_Lines(SpectrumSN):
    ...
    def __init__(self, spec1D, z, snr=10, bin=False, bin_size=1):
        """Constructor
        ...
        super(SpectrumSN_Lines, self).__init__(spec1D, z=z)
...
class AbsorbLine(SpectrumSN):
    ...
    def __init__(
        ...
        if bin:
            ...
            dat = data_binning(
                np.array([self.wv_rf, self.fl, self.fl_unc]).T, size=bin_size
            )
        ...
        blue_fl = super(AbsorbLine, self).get_flux_at_lambda(blue_edge, delta_l=delta_l)
        red_fl = super(AbsorbLine, self).get_flux_at_lambda(red_edge, delta_l=delta_l)
    ...
    def MCMC_sampler(
        ...
        with pm.Model() as GaussProfile:
            ...
            if len(vel_mean_mu) == n_lines:
                v_mean = pm.Normal("v_mean", mu=vel_mean_mu, sigma=vel_mean_sig)
            else:
                raise IndexError(
            ...
            ln_v_var = pm.Uniform(
                "ln_v_var",
                lower=ln_vel_var_lim[0],
                upper=ln_vel_var_lim[1],
                shape=(n_lines,),
            )
    ...
    def MCMC_emcee_sampler(
        ...
        warn(
            "The function MCMC_emcee_sampler() is deprecated. Use MCMC_sampler() instead",
            DeprecationWarning,
            stacklevel=2,
        )
        ...
        if normalize_unc != "":
            if normalize_unc == "LS":
                ...
                chi2 = self.chi2_MCMC
            norm_fac = (chi2 / len(self.vel_rf)) ** 0.5
        ...
        for i in range(ndim1):
            hist, bin_edges = np.histogram(samples[:, i], bins=50, density=True)
        ...
        for k in range(ndim // 3):
            print(
                "Velocity {}: {:.0f} pm {:.0f} km/s".format(
                    k + 1, self.theta_MCMC[2 + 3 * k], self.sig_theta_MCMC[2 + 3 * k]
                )
            )

        self.chi2_MCMC = neg_lnlike_gaussian_abs(
            self.theta_MCMC,
            self.rel_strength,
            self.lambda_0,
            self.blue_vel,
        ...
        if plot_mcmc:
            ...
            plot_MCMC(
                sampler=sampler,
                num_vel_com=len(self.rel_strength),
                nplot=20,
                samples=samples,
            )
        if plot_tau:
...
def calc_gauss(mean_vel, var_vel, amplitude, vel_rf):
    ...
    gauss = (
        amplitude
        / np.sqrt(2 * np.pi * var_vel)
        * np.exp(-0.5 * (vel_rf - mean_vel) ** 2 / var_vel)
    )
    return gauss


def flux_gauss(theta, rel_strength, lambda_0, blue_vel, red_vel, vel_rf, lines=[]):
    ...
    for k in range(len(theta) // 3):
        ...
        for rel_s, li in zip(rel_strength[k], lines[k][:-1]):
            ...
            model_flux += rel_s * calc_gauss(vel, var_vel, amplitude, vel_rf)
...
def lnlike_gaussian_abs(
    ...
    model_flux = flux_gauss(
        theta0, rel_strength, lambda_0, blue_vel, red_vel, vel_rf, lines
    )
    if type == "gaussian":
        lnl = (
            -0.5 * len(model_flux) * np.log(2 * np.pi)
            - np.sum(np.log(flux_unc))
            - 0.5 * np.sum((norm_flux - model_flux) ** 2 / flux_unc**2)
        )
    elif type == "chi2":
...
def lnprior(
    ...
    sig_lim = min((vel_flat[1] - vel_flat[0] - delta_vel) / 2, var_max**0.5)
    ...
    if len(var_prior) == 0:
        ...
        lnpvf = (
            ...
            - (mean_vel - mu_prior) ** 2 / 2 / var_prior
        )
        lnpvf = lnpvf.sum()
    vlim = -np.inf
    for k in range(len(mean_vel)):
        if not (
            vel_flat[0] + delta_vel < mean_vel[k] < vel_flat[1]
            and 2e1 < var_vel[k] ** 0.5 < sig_lim
            and (-((2 * np.pi * var_vel[k]) ** 0.5) * np.mean([y1, y2]) < amplitude[k])
            and mean_vel[k] > vlim
        ):
            return -np.inf
        vlim = mean_vel[k]
...
def plot_MCMC(sampler=None, num_vel_com=1, nburn=0, thin=1, nplot=None, samples=[]):
    ...
    paramsNames = [r"$\mathrm{Blue\ edge\ flux}$", r"$\mathrm{Red\ edge\ flux}$"]
    ...
    if len(samples) == []:
        samples = sampler.get_chain(discard=nburn, flat=True, thin=thin)
    fig = corner.corner(
        samples, labels=paramsNames, quantiles=[0.16, 0.50, 0.84], show_titles=True
    )
...
def plotChains(sampler, nburn, paramsNames, thin=1, nplot=None):
    ...
    fig, ax = plt.subplots(Nparams + 1, 1, figsize=(12, 3 * (Nparams + 1)), sharex=True)
    ...
    for i, p in enumerate(paramsNames):
        for w in selected_walkers:
            burn = ax[i].plot(
                xplot[: nburn // thin],
                sampler.get_chain(thin=thin)[: nburn // thin, w, i],
                alpha=0.4,
                lw=0.7,
                zorder=1,
            )
            ...
            if i == Nparams - 1:
                ax[i + 1].plot(
                    xplot[: nburn // thin],
                    sampler.get_log_prob(thin=thin)[: nburn // thin, w],
                    color=burn[0].get_color(),
                    alpha=0.4,
                    lw=0.7,
                    zorder=1,
                ...
                ax[i + 1].plot(
                    xplot[nburn // thin - 1 :],
                    sampler.get_log_prob(thin=thin)[nburn // thin - 1 :, w],
                    color=burn[0].get_color(),
                    alpha=0.8,
                    lw=0.7,
                    zorder=1,
